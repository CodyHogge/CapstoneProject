...
<c:set var="page" value="Embroidery"/>
<!DOCTYPE html>
<html>
<head>
         ...
         <li class="nav-item">
          ...
          <li class="nav-item">
            <a class="nav-link" href="embroidery">
              ...
              Embroidery <span class="sr-only">(current)</span>
            </a>
          </li>
          <li class="nav-item">
        ...
        <table class="table table-striped table-sm">
          ...
          <tbody>
            ...
            <c:if test="${order.status ne 'COMPLETE' }">
            	<c:if test="${order.div1 eq 'EMB' || order.div2 eq 'EMB' || order.div3 eq 'EMB'}">
            	<c:if test="${order.priority eq 'HIGH'}">
            		<tr>
            			...
            			<td>
          				...
          				<td>
          					<c:choose>
          						<c:when test="${role eq 'ADMIN' or division eq 'EMB' }">
            	...
            	<c:if test="${order.status eq 'NEW'}">
            		<tr>
          			...
          			<c:if test="${order.priority eq 'LOW'}">
          				...
          				<td>
          					<c:choose>
          						<c:when test="${role eq 'ADMIN' || division eq 'EMB' }">
          							<form method="POST" action="editOrderStatus">
											<input type="hidden" name="orderId"	value="${order.orderId}" />
											 <select onchange="this.form.submit()" name="status" class="form-control">
											<option value="${order.status}">${order.status}</option>
          				...
          				<td>
							...
							<i class="fa fa-sticky-note-o" aria-hidden="true"></i></a>
          					...
          					<i class="fa fa-check" aria-hidden="true"> Accept</i></a> --%>
          					
          					<a href="acceptOrder2?orderId=${order.orderId}" title="Accept Order"
